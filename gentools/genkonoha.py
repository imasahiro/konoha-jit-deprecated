...
class Class :
    ...
    def StructData(self):
        fmt = '''
\tDATA_STRUCT, CLASS_$C, _DATA(&$CSPI), FLAG_$C,'''
    ...
    def ClassData(self):
        fmt = '''
\tDATA_CLASS, _DATA("konoha.%s"), FLAG_%s, %s, %s, %s + %s /*%s*/,''' % (self.name, self.cname, CLASS_(self.base), CLASS_(self.parent), self.method_size, self.formatter_size, self.mapper_size)
        return fmt
...
class Translator:
    ...
    def TranslatorData(self):
        func = 'NULL'
        if self.meta.has_key('@Func'):
            func = self.meta['@Func']
        flag = '0'
        flag = addflag(flag, self.meta, 'Translator', '@Const')
        flag = addflag(flag, self.meta, 'Translator', '@Total')
        flag = addflag(flag, self.meta, 'Translator', '@Semantic')
        flag = addflag(flag, self.meta, 'Translator', '@Final')
        fmt = '''
\tDATA_TCAST, %s, %s, %s, _DATA(%s),''' % (CLASS_(self.fcname), CLASS_(self.tcname), flag, func)
        return fmt

def parse_Translator(meta, tokens, data):
    mpr = Translator(tokens[0], tokens[1])
    mpr.meta = meta
    if not tokens[1].endswith('?'):
        meta['@Total'] = True
    data.add_Translator(mpr)
...
\t{"%s", %s},''' % (fn, FN_(fn))
    ...
    for mpr in data.TCAST_LIST:
        dlist.append(mpr.TranslatorData())
...
def genkonoha():
    ...
    data.parser['mapper'] = parse_Translator
...
if __name__ == '__main__':
    ...
    gen_vm_c(".")
    from gen_gamma import gen_stmt
    gen_stmt(".")
