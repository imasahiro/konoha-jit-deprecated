...
def write_class0_h(f, c, n):
	...
	if(c.cname=='Any'):
		write_define(f, TYPE_cname(c.cname), 'NATYPE_cid(%s)' % CLASS_cname(c.cname), 32)
		write_define(f, 'NN' + TYPE_cname(c.cname), 'NATYPE_cid(%s)' % CLASS_cname(c.cname), 32)
	else:	
		write_define(f, TYPE_cname(c.cname), '%s' % CLASS_cname(c.cname), 32)
		write_define(f, 'NN' + TYPE_cname(c.cname), 'NNTYPE_cid(%s)' % CLASS_cname(c.cname), 32)
	write_define(f, 'NA' + TYPE_cname(c.cname), 'NATYPE_cid(%s)' % CLASS_cname(c.cname), 32)
...
def gen_class_h(bdir, data):
	...
	write_define(f, 'NAATYPE_Int', 'NATYPE_cid(CLASS_IArray)', 32)
	...
	write_define(f, 'NAATYPE_Float', 'NATYPE_cid(CLASS_FArray)', 32)
	...
	for cname in data.ACLASS_LIST:
		...
		write_define(f, 'NAATYPE_%s' % cname, 'NATYPE_cid(ACLASS_%s)' % cname, 32)
		cn += 1
	...
	for cname in data.ICLASS_LIST:
		...
		write_define(f, 'NAITYPE_%s' % cname, 'NNTYPE_cid(ICLASS_%s)' % cname, 32)
		cn += 1
	...
	for c in data.GENERICS_LIST:
		...
		write_define(f, 'NNTYPE_%s' % c.cname, 'NNTYPE_cid(CLASS_%s)' % c.cname, 32)
		write_define(f, 'NATYPE_%s' % c.cname, 'NATYPE_cid(CLASS_%s)' % c.cname, 32)
		cn += 1
	...
	for cname in data.XCLASS_LIST:
		...
		write_define(f, 'NATYPE_%s' % cname, 'NATYPE_cid(CLASS_%s)' % cname, 32)
		cn += 1
...
static knh_GenericsData_t GenericsData[] = {''')
	for cname in data.ICLASS_LIST:
		cname = cname.replace(":", "__")
		f.write('''
\t{"Iterator<%s>", ICLASS_%s, CLASS_Iterator, CLASS_%s, CLASS_unknown},''' % (cname, cname, cname))
	for cname in data.ACLASS_LIST:
		cname = cname.replace(":", "__")
		f.write('''
...
static knh_MethodFieldData_t MethodFieldData[] = {''')
	for n in data.MF_LIST:
		...
		func = func.replace(' TYPE_', ' NATYPE_').replace('NATYPE_void', 'TYPE_void')
		f.write(func)
	f.write('''
	{-1, 0}
};
